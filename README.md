...
- **🔒 Catches mistakes before they ship.** `scan-secrets` blocks any commit containing API keys or a real `.env`. `destructive-git-guard` blocks force push, `git reset --hard`, `rm -rf`, and other irreversible commands until you confirm them yourself. `stop-verify` runs lint, typecheck, and related tests on the files you changed at the end of every turn and won't let "done" slide if they fail. `check-dep` forces a real look at any new dependency (size, maintenance, lighter alternatives) before it's added.
...
- **`hooks/`**
  - `caveman-activate.sh` — terse response style on session start (derived from the caveman skill, single source of truth)
  - `scan-secrets.sh` — blocks `git commit` if staged diff contains secrets/`.env`
  - `destructive-git-guard.sh` — blocks force push, `git reset --hard`, `git clean -f`, `git branch -D`, `git checkout`/`restore` of uncommitted work, `rm -rf`, and `--no-verify`/`--no-gpg-sign`, until you confirm
  - `check-dep.sh` — prompts to research a dependency before adding it
  - `eslint-fix.sh` — auto `eslint --fix` on edited JS/TS files
  - `stop-verify.sh` — lint, typecheck, and run related tests on changed JS/TS files when a turn ends
